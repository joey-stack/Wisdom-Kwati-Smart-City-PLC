...
export default function AdminEditCareerPage({ params }) {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          setFormData({
            ...
            workingHours: data.workingHours || 'Standard business hours',
            reportsTo: data.reportsTo || '',
            howToApply: data.howToApply || 'careers@wisdomkwati.com'
          });

          // Join arrays for textareas
          if (data.responsibilities) {
            setResponsibilitiesText(data.responsibilities.join('\n'));
          }
          if (data.perks) {
            setPerksText(data.perks.join('\n'));
          }
          if (data.requirements && data.requirements.length > 0) {
            setRequirements(data.requirements);
          }
        } else {
          setError('Job listing not found in database.');
        }
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError('Error saving job changes to database.');
      setSubmitting(false);
    }
  };
    ...
    <div style={{ maxWidth: '900px', margin: '0 auto' }}>
      <header className="admin-header" style={{ marginBottom: '32px' }}>
        <div className="admin-title-group">
          <h1>Edit Job Profile</h1>
          <p>Update requirements, position details, or instructions for {formData.title}.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
            ...
            <div className="form-group">
              ...
              <input
                ...
                required
                disabled
                style={{ width: '100%', padding: '12px 16px', borderRadius: '4px', border: '1px solid var(--admin-border)', backgroundColor: 'var(--admin-bg)', color: 'var(--admin-text-primary)', fontSize: '13px', outline: 'none', opacity: 0.6, cursor: 'not-allowed' }}
              />
            </div>
          </div>
          ...
          <div className="form-group">
            ...
            <textarea
              ...
              onChange={handleChange}
              rows="3"
              placeholder="e.g. We are looking for a Marketing Coordinator to help manage and grow the WKSC brand..."
          ...
          >
            {submitting ? 'Saving Position Changes...' : 'Save Job Changes'}
          </button>
        </form>
      </div>
    </div>
