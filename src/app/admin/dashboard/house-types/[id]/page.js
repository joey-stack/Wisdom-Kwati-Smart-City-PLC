...
export default function AdminEditHouseTypePage({ params }) {
  ...
  useEffect(() => {
    async function loadHouseTypeData() {
      try {
        ...
        if (docSnap.exists()) {
          ...
          setBeds(data.beds !== undefined ? String(data.beds) : '');
          setBaths(data.baths !== undefined ? String(data.baths) : '');
          setSize(data.size || '');
          setLotSize(data.lotSize || '');
          setBuiltIn(data.builtIn || '');
          setFloors(data.floors !== undefined ? String(data.floors) : '');
          ...
          setAmenitiesNote(data.amenitiesNote || '');
          setInteriorNote(data.interiorNote || '');
          setAdvisorId(data.advisorId || '');
          setSortOrder(data.sortOrder !== undefined && data.sortOrder !== null ? String(data.sortOrder) : '');

          // Map images array to individual strings
          ...
          setError('Specification record not found in database.');
        }
      } catch (err) {
        console.error('Error fetching house spec:', err);
        setError('Error loading specification record.');
      } finally {
        setLoading(false);
      }
    }
    ...
    // Load all advisors for the selector dropdown
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!classType) {
      setError('Please provide a Class/Model Title.');
      return;
    }
    ...
    try {
      ...
      await updateDoc(docRef, houseTypePayload);
      router.push('/admin/dashboard/house-types');
    } catch (err) {
      console.error('Failed to save changes:', err);
      setError('Failed to update specification records.');
      setSubmitting(false);
    }
  };
    ...
    <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
      <header className="admin-header" style={{ marginBottom: '32px' }}>
        <div className="admin-title-group">
          <h1>Edit Villa Specification</h1>
          <p>Update model sizes, room layout indicators, dynamic amenities tags, and bento collections for {classType}.</p>
        </div>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
        ...
        <div className="admin-section-card" style={{ padding: '32px' }}>
          ...
          <div style={{ flex: 'column', display: 'flex', gap: '16px' }}>
        ...
        >
          {submitting ? 'Saving Spec Changes...' : 'Save Spec Changes'}
        </button>
