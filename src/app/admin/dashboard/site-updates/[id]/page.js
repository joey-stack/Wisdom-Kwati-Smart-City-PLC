...
export default function AdminEditSiteUpdatePage({ params }) {
  ...
  // New media items undergoing compression & upload
  ...
  // Keep ref of projectId for uploads
  ...
  useEffect(() => {
    async function loadFormData() {
      try {
        ...
        const list = projSnap.docs.map(doc => ({
          id: doc.id,
          name: doc.data().name || doc.id
        }));
        setProjects(list);

        const updateRef = doc(db, 'siteUpdates', id);
        ...
        if (updateSnap.exists()) {
          ...
          setError('Site update record not found.');
        }
      } catch (err) {
        console.error('Failed to load data:', err);
        setError('Error loading site update details from database.');
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const existingImages = formData.images.filter(url => url && url.trim() !== '');
    ...
    if (anyUploading) {
      setError('Please wait for all media files to finish uploading before saving.');
      return;
    }
    ...
    const finalImages = [...existingImages, ...completedUrls, ...pastedUrls];

    if (finalImages.length === 0) {
      setError('Please provide at least one valid image or video file or paste a valid media URL.');
      return;
    }
    ...
    try {
      const docRef = doc(db, 'siteUpdates', id);
      const { images, ...rest } = formData;
      await updateDoc(docRef, {
        ...rest,
        images: finalImages,
        image: finalImages[0] || '', // Backwards compatibility
        updatedAt: new Date().toISOString()
      });
      router.push('/admin/dashboard/site-updates');
    } catch (err) {
      console.error('Failed to update site update:', err);
  ...
  const handleDelete = async () => {
    if (!confirm(`Are you sure you want to delete this progress update: "${formData.title}"? This action cannot be undone.`)) {
      return;
    }
    ...
    try {
      ...
      setError('Failed to delete progress update from database.');
      setSubmitting(false);
    }
  };
    ...
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      <header className="admin-header" style={{ marginBottom: '32px' }}>
        <div className="admin-title-group">
          <h1>Edit Progress Update</h1>
          <p>Modify update text description, headline, or linked images.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div className="form-group" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <label style={{ display: 'block', fontSize: '11px', fontWeight: 600, color: 'var(--admin-text-secondary)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '4px' }}>
              Progress Media * (Existing and New)
            </label>
            
            {/* Existing Images */}
            {formData.images.length > 0 && (
              <div>
                <p style={{ fontSize: '12px', color: 'var(--admin-text-secondary)', marginBottom: '8px' }}>Currently Saved Media:</p>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px' }}>
                  {formData.images.map((url, index) => {
                    const resolvedUrl = resolveMediaUrl(url);
                    const isVid = url.toLowerCase().includes('.mp4') || url.toLowerCase().includes('.webm') || url.toLowerCase().includes('.mov');
                    return (
                      <div key={`existing-${index}`} style={{ position: 'relative', width: '100px', height: '100px', borderRadius: '4px', overflow: 'hidden', border: '1px solid var(--admin-border)', backgroundColor: 'var(--admin-surface-light)' }}>
                        {isVid ? (
                          <video
                            src={resolvedUrl}
                        ...
                        <button
                          ...
                          style={{
                          position: 'absolute', top: '4px', right: '4px', background: 'rgba(239, 68, 68, 0.9)', color: '#fff', border: 'none', borderRadius: '50%', width: '20px', height: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: '12px', padding: 0
                        }}
            ...
            <p style={{ fontSize: '12px', color: 'var(--admin-text-secondary)', marginTop: '8px', marginBottom: '4px' }}>Add New Media (Optimizes & uploads instantly):</p>
            <input 
              type="file" 
              multiple 
              accept="image/*,video/*" 
            ...
            <div style={{ marginTop: '16px', borderTop: '1px dashed var(--admin-border)', paddingTop: '16px' }}>
              <label style={{ display: 'block', fontSize: '11px', fontWeight: 600, color: 'var(--admin-text-secondary)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '8px' }}>
                OR Paste New Media URLs (Fallback / No Upload Required)
              </label>
              <textarea
                value={mediaUrlsText}
              ...
              <span style={{ fontSize: '11px', color: 'var(--admin-text-secondary)', marginTop: '4px', display: 'block' }}>
                Use this option to add more media URLs if you cannot upload files due to Firebase Storage plan limits.
              </span>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '16px', marginTop: '12px' }}>
            ...
            >
              {submitting ? 'Saving Changes...' : mediaItems.some(item => item.status === 'compressing' || item.status === 'uploading') ? 'Uploading new files...' : 'Save Update Changes'}
            </button>
            <button
              type="button"
