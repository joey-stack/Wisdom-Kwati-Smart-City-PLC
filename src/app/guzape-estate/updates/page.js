...
export default function Page() {
    const assets = projectAssets.guzape.updates;
        ...
        <main>
            ...
            <section className="pj-hero-section" style={{ paddingTop: "180px", paddingBottom: "60px", background: "var(--bg-main)" }}>
                <div className="container container-1380" style={{ textAlign: "center", maxWidth: "800px", margin: "0 auto" }}>
                    ...
                    <Link href="/guzape-estate" className="btn-pill" style={{ marginTop: "10px", display: "inline-flex", background: "var(--accent-green)", color: "var(--text-primary)", border: "none" }}>
                ...
                <div className="gallery-grid reveal-on-scroll">
                    {assets.map((id, index) => (
                        <div key={index} className="gallery-item skeleton">
                            {id === "1sNiIK0VCsxAS7GTBv8aTSoF6vDPYRv1a" || id === projectAssets.guzape.heroVideo || id === "1WkaEVNo0ii8zkmYXHDOd5MOFwDcz7VKi" ? (
                                <video autoPlay src={`https://drive.google.com/uc?export=download&id=${id}`} poster={getWeservUrl(projectAssets.guzape.main)} controls muted loop playsInline preload="metadata" />
                            ) : (
                                <img loading="lazy" src={getWeservUrl(id)} alt={`Nimi Hills Site Update ${index + 1}`} referrerPolicy="no-referrer" />
                            )}
                        </div>
                    ))}
                </div>
