...
export default function Page() {
    const assets = projectAssets.karsana.updates;
        ...
        <main>
            ...
            <section className="pj-hero-section" style={{ paddingTop: "180px", paddingBottom: "60px", background: "var(--bg-main)" }}>
                <div className="container container-1380" style={{ textAlign: "center", maxWidth: "800px", margin: "0 auto" }}>
                    ...
                    <Link href="/karsana-district" className="btn-pill" style={{ marginTop: "10px", display: "inline-flex", background: "var(--accent-green)", color: "var(--text-primary)", border: "none" }}>
                    ...
                    {assets.map((id, index) => (
                        <div key={index} className="gallery-item skeleton">
                            {id === "1WkaEVNo0ii8zkmYXHDOd5MOFwDcz7VKi" ? (
                                <video autoPlay src={`https://drive.google.com/uc?export=download&id=${id}`} poster={getWeservUrl(projectAssets.karsana.main)} controls muted loop playsInline preload="metadata" />
                            ) : (
                                <img loading="lazy" src={getWeservUrl(id)} alt={`Karsana Site Update ${index + 1}`} referrerPolicy="no-referrer" />
                            )}
                        </div>
                    ))}
                </div>
