...
export default function Page() {
    const assets = projectAssets.mabushi.updates;
        ...
        <main>
            ...
            <section className="pj-hero-section" style={{ paddingTop: "180px", paddingBottom: "60px", background: "var(--bg-main)" }}>
                <div className="container container-1380" style={{ textAlign: "center", maxWidth: "800px", margin: "0 auto" }}>
                    ...
                    <Link href="/mabushi-district" className="btn-pill" style={{ marginTop: "10px", display: "inline-flex", background: "var(--accent-green)", color: "var(--text-primary)", border: "none" }}>
                ...
                <div className="gallery-grid reveal-on-scroll">
                    {assets.map((id, index) => (
                        <div key={index} className="gallery-item skeleton">
                            <img loading="lazy" src={getWeservUrl(id)} alt={`Mabushi Site Update ${index + 1}`} referrerPolicy="no-referrer" />
                        </div>
                    ))}
                </div>
            </main>
