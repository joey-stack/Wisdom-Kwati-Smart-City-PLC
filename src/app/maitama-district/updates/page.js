...
export default function Page() {
    const assets = projectAssets.maitama.updates;
        ...
        <main>
            ...
            <section className="pj-hero-section" style={{ paddingTop: "180px", paddingBottom: "60px", background: "var(--bg-main)" }}>
                <div className="container container-1380" style={{ textAlign: "center", maxWidth: "800px", margin: "0 auto" }}>
                    ...
                    <Link href="/maitama-district" className="btn-pill" style={{ marginTop: "10px", display: "inline-flex", background: "var(--accent-green)", color: "var(--text-primary)", border: "none" }}>
            ...
            <main className="container-1380" style={{ padding: "60px 20px" }}>
                ...
                <div style={{ marginTop: "40px" }} className="reveal-on-scroll">
                    ...
                    <p style={{ color: "var(--text-secondary)", maxWidth: "600px" }}>Explore the ongoing infrastructural developments and architectural milestones at Maitama District (Kwati City).</p>
                </div>

                <div className="gallery-grid reveal-on-scroll">
                    {assets.map((id, index) => (
                        <div key={index} className="gallery-item skeleton">
                            <img loading="lazy" src={getWeservUrl(id)} alt={`Maitama Site Update ${index + 1}`} referrerPolicy="no-referrer" />
                        </div>
                    ))}
                </div>
            </main>
