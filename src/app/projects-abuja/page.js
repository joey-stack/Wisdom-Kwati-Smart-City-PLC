...
// Global in-memory cache for Abuja projects
let globalAbujaProjectsCache = null;

export default function Page() {
  const [projects, setProjects] = useState(() => globalAbujaProjectsCache || []);
  const [loading, setLoading] = useState(() => !globalAbujaProjectsCache);
    ...
    async function loadProjects() {
      try {
        ...
        snap.forEach((doc) => {
          ...
          // Filter for Abuja
          if (!state.includes('abuja') && !state.includes('fct') && doc.id !== 'beverly-hills') return;
          list.push({
            id: doc.id,
            name: d.name,
        ...
        globalAbujaProjectsCache = list;
        setProjects(list);
      } catch (err) {
        console.error('Error fetching Abuja projects:', err);
      } finally {
        setLoading(false);
      }
    }
          ...
          <div className="container container-1380">
            {!mounted || loading ? (
              ...
              <p style={{ textAlign: 'center', padding: '60px 0', fontFamily: 'var(--font-main)', fontSize: '15px', color: 'var(--text-secondary)' }}>
                No projects found in Abuja district.
              </p>
            ) : (
              <div className="wksc-proj-grid" id="project-grid">
                {projects.map((proj) => (
