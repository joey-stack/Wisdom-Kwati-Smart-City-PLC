...
// Global in-memory cache for Lagos projects
let globalLagosProjectsCache = null;

export default function Page() {
  const [projects, setProjects] = useState(() => globalLagosProjectsCache || []);
  const [loading, setLoading] = useState(() => !globalLagosProjectsCache);
    ...
    async function loadProjects() {
      try {
        ...
        snap.forEach((doc) => {
          ...
          // Filter for Lagos / Epe
          if (!state.includes('lagos') && !state.includes('epe') && doc.id !== 'lagos-ekpe' && doc.id !== 'epe-smart-city') return;
          list.push({
            id: doc.id,
            name: d.name,
        ...
        globalLagosProjectsCache = list;
        setProjects(list);
      } catch (err) {
        console.error('Error fetching Lagos projects:', err);
      } finally {
        setLoading(false);
      }
    }
        ...
        <section className="pj-hero-section">
          <div className="container container-1380" style={{ textAlign: "center", maxWidth: "800px", margin: "0 auto" }}>
            ...
            <h1 className="pj-hero-title" style={{ fontFamily: "'Inter Display', 'Inter Display Placeholder', sans-serif", fontSize: "40px", fontStyle: "normal", fontWeight: "500", lineHeight: "44px", color: "var(--text-primary)", marginBottom: "64px" }}>
              Strategic coastal developments and<br />
              smart communities in the heart of Lagos.
            </h1>
          </div>
        </section>
          ...
          <div className="container container-1380">
            {!mounted || loading ? (
              ...
              <p style={{ textAlign: 'center', padding: '60px 0', fontFamily: 'var(--font-main)', fontSize: '15px', color: 'var(--text-secondary)' }}>
                No projects found in Lagos district.
              </p>
            ) : (
              <div className="wksc-proj-grid" id="project-grid">
                {projects.map((proj) => (
