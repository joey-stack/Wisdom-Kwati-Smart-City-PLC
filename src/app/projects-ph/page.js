...
// Global in-memory cache for Rivers State projects
let globalPhProjectsCache = null;

export default function Page() {
  const [projects, setProjects] = useState(() => globalPhProjectsCache || []);
  const [loading, setLoading] = useState(() => !globalPhProjectsCache);
    ...
    async function loadProjects() {
      try {
        ...
        snap.forEach((doc) => {
          ...
          // Filter for Rivers State / Port Harcourt
          if (!state.includes('rivers') && !state.includes('port harcourt') && !state.includes('ph') && doc.id !== 'ph-rumu-olumeni' && doc.id !== 'port-harcourt-estate') return;
          list.push({
            id: doc.id,
            name: d.name,
        ...
        globalPhProjectsCache = list;
        setProjects(list);
      } catch (err) {
        console.error('Error fetching Rivers State projects:', err);
      } finally {
        setLoading(false);
      }
    }
          ...
          <div className="container container-1380">
            {!mounted || loading ? (
              ...
              <p style={{ textAlign: 'center', padding: '60px 0', fontFamily: 'var(--font-main)', fontSize: '15px', color: 'var(--text-secondary)' }}>
                No projects found in Rivers State.
              </p>
            ) : (
              <div className="wksc-proj-grid" id="project-grid">
                {projects.map((proj) => (
